...
{
    public class NewsTypeDAL : INewsTypeDAL
    {
        public bool Exists(string name)
        {
            string sql = "select count(1)num from tbNewsType where ftypename = @ftypename";
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from tbNewsType ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
        ...
        public bool DeleteList(string idlist)
        {
            ...
            list.Add("delete from tbNewsType where Id in (" + idlist + ")");
