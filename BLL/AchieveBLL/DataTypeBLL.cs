...
namespace AchieveBLL
{
    ...
    {
        IDataTypeDAL dal = DALFactory.GetDataTypeDAL();
        public DataTypeBLL()
        { }
        ...
        public string GetAllDataTypeInfo(string where)
        {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(DataTypeEntity model)
        {
        ...
        /// 更新一条数据
        /// </summary>
        public int Update(DataTypeEntity model)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public DataTypeEntity GetModel(int id)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<DataTypeEntity> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<DataTypeEntity> DataTableToList(DataTable dt)
        {
            List<DataTypeEntity> modelList = new List<DataTypeEntity>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                DataTypeEntity model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new DataTypeEntity();
                    if (dt.Rows[n]["id"].ToString() != "")
                    {
                        model.id = int.Parse(dt.Rows[n]["id"].ToString());
                    }
                    model.DataType = dt.Rows[n]["DataType"].ToString();
                    model.DataTypeName = dt.Rows[n]["DataTypeName"].ToString();
