...
namespace AchieveBLL
{
    public class TableBLL
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(TableEntity model)
        {
        ...
        /// 更新一条数据
        /// </summary>
        public int Update(TableEntity model)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public TableEntity GetModel(int id)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<TableEntity> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<TableEntity> DataTableToList(DataTable dt)
        {
            List<TableEntity> modelList = new List<TableEntity>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                AchieveEntity.TableEntity model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new TableEntity();
                    if (dt.Rows[n]["Id"].ToString() != "")
                    {
                        model.Id = int.Parse(dt.Rows[n]["Id"].ToString());
                    }
                    model.TabName = dt.Rows[n]["TabName"].ToString();
                    model.TabViewName = dt.Rows[n]["TabViewName"].ToString();
